						...
						<?php wp_nonce_field( 'wpmem-update-settings' ); ?>
							...
							<ul>
								<li>
									...
									<input name="wpmem_block_post" type="checkbox" id="" value="post" <?php if ( $wpmem_settings['block']['post'] == 1 ) { echo "checked"; }?> /> <span>Posts</span>
								</li>
								<li style="border-bottom:1px solid #eee;">
									<label>&nbsp;</label>
									<input name="wpmem_block_page" type="checkbox" id="" value="page" <?php if ( $wpmem_settings['block']['page'] == 1 ) { echo "checked"; }?> /> <span>Pages</span>
								</li>
								<li>
									<label>Show Excerpts</label>
									<input name="wpmem_show_excerpt_post" type="checkbox" id="" value="post" <?php if ( $wpmem_settings['show_excerpt']['post'] == 1 ) { echo "checked"; }?> /> <span>Posts</span>
								</li>
								<li style="border-bottom:1px solid #eee;">
									<label>&nbsp;</label>
									<input name="wpmem_show_excerpt_page" type="checkbox" id="" value="page" <?php if ( $wpmem_settings['show_excerpt']['post'] == 1 ) { echo "checked"; }?> /> <span>Pages</span>
								</li>
								<li>
									<label>Show Login Form</label>
									<input name="wpmem_show_login_post" type="checkbox" id="" value="1" <?php if ( $wpmem_settings['show_login']['post'] == 1 ) { echo "checked"; }?> /> <span>Posts</span>
								</li>
								<li style="border-bottom:1px solid #eee;">
									<label>&nbsp;</label>
									<input name="wpmem_show_login_page" type="checkbox" id="" value="1" <?php if ( $wpmem_settings['show_login']['page'] == 1 ) { echo "checked"; }?> /> <span>Pages</span>
								</li>
								<li>
									<label>Show Registration Form</label>
									<input name="wpmem_show_reg_post" type="checkbox" id="" value="1" <?php if ( $wpmem_settings['show_reg']['post'] == 1 ) { echo "checked"; }?> /> <span>Posts</span>
								</li>
								<li>
									<label>&nbsp;</label>
									<input name="wpmem_show_reg_page" type="checkbox" id="" value="1" <?php if ( $wpmem_settings['show_reg']['page'] == 1 ) { echo "checked"; }?> /> <span>Pages</span>
								</li>
							</ul>
							<?php
							...
							for ( $row = 0; $row < count( $arr ); $row++ ) { ?>
							  <li>
								...
								<?php if (WPMEM_DEBUG == true) { echo $wpmem_settings[$arr[$row][3]]; } ?>
								<input name="<?php echo $arr[$row][1]; ?>" type="checkbox" id="<?php echo $arr[$row][1]; ?>" value="1" <?php if ( $wpmem_settings[$arr[$row][3]] == 1 ) { echo "checked"; }?> />&nbsp;&nbsp;
							...
							<?php } ?>
							<?php $attribution = $wpmem_settings['attrib']; ?>
							  <li>
							...
							<?php $auto_ex = $wpmem_settings['autoex']; ?>
							  <li>
							  ...
							  <li>
								...
								<select name="wpmem_settings_captcha">
									<option value="0"<?php echo ( $wpmem_settings['captcha'] == 0 ) ? ' selected ' : ''; ?>><?php _e( 'None', 'wp-members' ); ?></option>
									<option value="1"<?php echo ( $wpmem_settings['captcha'] == 1 ) ? ' selected ' : ''; ?>>reCAPTCHA</option>
									<?php // if rs captcha is enabled ?>
									<option value="2"<?php echo ( $wpmem_settings['captcha'] == 2 ) ? ' selected ' : ''; ?>>Really Simple CAPTCHA</option>
								</select>
							  </li>
							<h3><?php _e( 'Pages' ); ?></h3>
							  <?php $wpmem_logurl = $wpmem_settings['user_pages']['login'];
							  ...
							  <?php $wpmem_regurl = $wpmem_settings['user_pages']['register'];
							  ...
							  <?php $wpmem_msurl = $wpmem_settings['user_pages']['profile'];
							...
							<h3><?php _e( 'Stylesheet' ); ?></h3>
							  <li>
								...
								<?php wpmem_admin_style_list( $wpmem_settings['style'] ); ?>
								</select>
							  </li>							  
							  <?php $wpmem_cssurl = $wpmem_settings['cssurl'];
...
function wpmem_update_options() {
	...
	$wpmem_newsettings = array(
		...
		'block'   => array(
			'post' => ( isset( $_POST['wpmem_block_post'] ) ) ? 1 : 0,
			'page' => ( isset( $_POST['wpmem_block_page'] ) ) ? 1 : 0,
		),
		'show_excerpt' => array(
			'post' => ( isset( $_POST['wpmem_show_excerpt_post'] ) ) ? 1 : 0,
			'page' => ( isset( $_POST['wpmem_show_excerpt_page'] ) ) ? 1 : 0,
		),
		'show_reg' => array(
			'post' => ( isset( $_POST['wpmem_show_reg_post'] ) ) ? 1 : 0,
			'page' => ( isset( $_POST['wpmem_show_reg_page'] ) ) ? 1 : 0,
		),
		'show_login' => array(
			'post' => ( isset( $_POST['wpmem_show_login_post'] ) ) ? 1 : 0,
			'page' => ( isset( $_POST['wpmem_show_login_page'] ) ) ? 1 : 0,
		),
		'notify'    => ( isset( $_POST['wpmem_settings_notify'] ) ) ? 1 : 0,
		'mod_reg'   => ( isset( $_POST['wpmem_settings_moderate'] ) ) ? 1 : 0,
		'captcha'   => ( isset( $_POST['wpmem_settings_captcha'] ) ) ? $_POST['wpmem_settings_captcha'] : 0,
		'use_exp'   => ( isset( $_POST['wpmem_settings_time_exp'] ) ) ? 1 : 0,
		'use_trial' => ( isset( $_POST['wpmem_settings_trial']    ) ) ? 1 : 0,
		'warnings'  => ( isset( $_POST['wpmem_settings_ignore_warnings'] ) ) ? 1 : 0,
		'user_pages' => array(
			'profile'  => ( $msurl  ) ? $msurl  : '',
		...
		'autoex'    => $wpmem_autoex,
		'attrib'    => ( isset( $_POST['attribution'] ) ) ? 1 : 0,
