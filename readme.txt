...
WP-Members is a free membership management system for WordPress® to restrict content to be viewable by registered members.  Unlike other plugins, it puts the registration and login process inline with your theme and content rather than directing users to the WP login. 
...
* To begin restricting pages, change the plugin default setting for pages to be blocked. The `<!--more-->` link is not necessary in the blocking of pages, but can be used if you have the "show excerpts" setting turned on.
...
The registration fields used by the plugin are stored in an array in the WP options table.  A description of how to change this is here: http://butlerblog.com/2010/09/27/adding-custom-fields-in-wp-members/
